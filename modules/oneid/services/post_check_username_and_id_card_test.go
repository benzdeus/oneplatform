...
func TestPostCheckUsernameIdCard(t *testing.T) {
	...
	t.Run("ShouldBeSuccess", func(t *testing.T) {
		response := service.PostCheckUsernameAndIdCard("yeansinizter", "111111")
		if response.Data == false {
			t.Error(response.Result)
		}
	})
	t.Run("ShouldBeFail", func(t *testing.T) {
		response := service.PostCheckUsernameAndIdCard("yeansinizter", "1111111")
